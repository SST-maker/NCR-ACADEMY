...
function documentsView(role) {
  return appShell(template`
    <section class="glass-panel content-card fade-in">
      ...
      <p class="lead small">Les modèles inclus sont prêts pour une mise à disposition. En production, les documents sensibles devront passer par un stockage sécurisé.</p>
      <div class="document-grid">
        ${documentTemplates.map(function renderDoc(doc) { return `<article><span>${escapeHTML(doc.type)}</span><strong>${escapeHTML(doc.name)}</strong><p>${escapeHTML(doc.status)}</p>${doc.href ? `<a class="ghost-button doc-link" href="${escapeHTML(doc.href)}" download>Télécharger</a>` : '<button class="ghost-button" disabled>À connecter</button>'}</article>`; }).join('')}
      </div>
    </section>
  `, role || 'student');
}
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-action="logout"]').forEach(function bindLogout(button) {
    button.addEventListener('click', function doLogout() {
      addEvent('logout', { user: session && session.email ? session.email : 'unknown' });
      logout();
      session = null;
      drawerOpen = false;
      navigate('/login');
  ...
  document.querySelectorAll('[data-course-id]').forEach(function bindCourse(button) {
    button.addEventListener('click', function changeCourse() {
      ...
      drawerOpen = false;
      addEvent('course_selected', { courseId: button.dataset.courseId });
      render();
    });
  });
  ...
  if (quizForm) quizForm.addEventListener('submit', function submitQuiz(event) {
    ...
    addEvent('quiz_submitted', { courseId: course.id, score: score, certified: score >= 80 });
    render();
  });
