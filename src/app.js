    ...
    <main class="login-page">
      <section class="login-card glass-panel" aria-labelledby="login-title">
        ...
        <form id="login-form" class="auth-form">
          <label>Email
            <input name="email" type="email" autocomplete="username" placeholder="stagiaire@ncr.demo" required />
          </label>
          <label>Mot de passe
            <input name="password" type="password" autocomplete="current-password" placeholder="ncr2026" required />
          </label>
...
function learningView() {
  ...
  return shell(h`
    ...
    <section class="section-grid two">
      ...
      <article class="glass-panel content-card">
        <div class="section-heading"><p class="eyebrow">Qualiopi</p><h3>Clarté du parcours</h3></div>
        <ul class="soft-list">
          <li>Objectifs visibles avant chaque module</li>
          <li>Accessibilité et adaptation prévues</li>
          <li>Évaluations rattachées au parcours</li>
        </ul>
      </article>
    </section>
      ...
      ${course.modules.map((module, index) => `
        <article class="module-card glass-panel ${completed.includes(index) ? 'done' : ''}">
          ...
          <div>
            ...
            <ul>${module.bullets.map(item => `<li>${escapeHTML(item)}</li>`).join('')}</ul>
...
function memoView() {
  ...
  return shell(h`
    <section class="section-grid two">
      <article class="glass-panel content-card">
        <div class="section-heading"><p class="eyebrow">Révision rapide</p><h2>Fiche mémo</h2></div>
        <ul class="memo-list">${course.memo.map(item => `<li>${escapeHTML(item)}</li>`).join('')}</ul>
      </article>
      <article class="glass-panel content-card accent-card">
        <div class="section-heading"><p class="eyebrow">Source pédagogique</p><h3>${escapeHTML(course.source)}</h3></div>
        <p>Cette zone est prête à recevoir les fiches condensées par module, téléchargeables ou consultables hors ligne.</p>
      </article>
    </section>
  `);
}
    ...
    <section class="section-grid two">
      <article class="glass-panel content-card">
        ...
        <form id="ticket-form" class="auth-form">
          <label>Sujet<input name="subject" placeholder="Ex : question sur le module 2" required /></label>
          <label>Message<textarea name="message" rows="5" placeholder="Décris ton besoin simplement" required></textarea></label>
...
function documentsView(role = 'student') {
  return shell(h`
    <section class="glass-panel content-card">
      ...
      <p class="lead small">La structure est prête pour connecter Firebase Storage, Supabase Storage ou un espace privé. Les fichiers sensibles ne doivent pas être exposés dans un dépôt GitHub public.</p>
      <div class="document-grid">
        ${documentTemplates.map(doc => `<article><span>${escapeHTML(doc.type)}</span><strong>${escapeHTML(doc.name)}</strong><p>${escapeHTML(doc.status)}</p><button class="ghost-button" disabled>À connecter</button></article>`).join('')}
      </div>
    </section>
  `, role);
}
