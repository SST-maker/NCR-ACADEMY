...
async function buildConvocationPdf(session, studentProfile, trainerSession) {
  ...
  const contactEmail = trainerSession.email || 'ncr-formations@outlook.fr';

  ctx.fillStyle = '#f5f8fe';
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  const bgGradient = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
  bgGradient.addColorStop(0, '#eff5ff');
  bgGradient.addColorStop(1, '#f9fbff');
  ...
  ctx.beginPath();
  ctx.arc(1080, 120, 220, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.arc(130, 1580, 190, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;

  const headerGradient = ctx.createLinearGradient(60, 60, 1180, 340);
  headerGradient.addColorStop(0, '#0b2f63');
  headerGradient.addColorStop(0.55, '#154889');
  headerGradient.addColorStop(1, '#2b6cb0');
  ctx.fillStyle = headerGradient;
  roundedRect(ctx, 60, 58, 1120, 292, 40);
  ctx.fill();
  ...
  if (logo) {
    ...
    ctx.drawImage(logo, 88, 92, targetWidth, targetHeight);
  }

  ctx.fillStyle = 'rgba(255,255,255,0.14)';
  roundedRect(ctx, 902, 90, 220, 42, 21);
  ctx.fill();
  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 16px Arial';
  ctx.fillText('DOCUMENT NOMINATIF', 935, 117);

  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 42px Arial';
  ctx.fillText('N.C.R Solutions', 88, 238);
  ctx.font = '22px Arial';
  ctx.globalAlpha = 0.95;
  ctx.fillText('Formations & Résolutions', 88, 274);
  ctx.globalAlpha = 1;

  ctx.textAlign = 'right';
  ctx.font = 'bold 50px Arial';
  ctx.fillText('Convocation', 1128, 178);
  ctx.font = 'bold 26px Arial';
  ctx.fillText('de formation', 1128, 214);
  ctx.font = '18px Arial';
  ctx.globalAlpha = 0.92;
  ctx.fillText(`Édition du ${generatedDate}`, 1128, 258);
  ctx.globalAlpha = 1;
  ctx.textAlign = 'left';

  roundedRect(ctx, 60, 386, 1120, 134, 28);
  ctx.fillStyle = '#ffffff';
  ctx.fill();
  ctx.strokeStyle = '#d7e4f3';
  ctx.lineWidth = 2;
  ctx.stroke();
  ctx.fillStyle = '#0f3b76';
  ctx.font = 'bold 20px Arial';
  ctx.fillText('Bienvenue dans votre prochaine session', 96, 430);
  ctx.fillStyle = '#425466';
  ctx.font = '21px Arial';
  drawParagraph(ctx, `${studentName}, vous êtes convoqué(e) à la formation « ${session.title || courseTitle} ». Retrouvez ci-dessous toutes les informations essentielles pour préparer votre venue dans les meilleures conditions.`, 96, 470, 1040, 30, '#425466');

  drawLabeledLines(ctx, 'Participant', [
    `Nom : ${studentName}`,
    `Email : ${studentProfile.email || 'Non précisé'}`
  ], { x: 60, y: 554, width: 520, height: 170, fill: '#ffffff', stroke: '#d6e2f2' });

  drawLabeledLines(ctx, 'Référence session', [
    `Formation : ${courseTitle}`,
    `Session : ${session.title || courseTitle}`,
    `Formateur : ${trainerName}`
  ], { x: 660, y: 554, width: 520, height: 170, fill: '#ffffff', stroke: '#d6e2f2' });

  roundedRect(ctx, 60, 756, 1120, 248, 30);
  ctx.fillStyle = '#ffffff';
  ctx.fill();
  ...
  ctx.fillText('Organisation de la formation', 92, 804);

  const infoCards = [
    { title: 'Dates', value: dateText, x: 92, y: 834, w: 240 },
    { title: 'Horaires', value: schedule, x: 352, y: 834, w: 240 },
    { title: 'Lieu', value: session.location || 'Lieu à confirmer', x: 612, y: 834, w: 260 },
    { title: 'Contact', value: contactEmail, x: 892, y: 834, w: 240 }
  ];
  infoCards.forEach(function drawCard(card) {
    roundedRect(ctx, card.x, card.y, card.w, 128, 22);
    ctx.fillStyle = '#f7faff';
    ctx.fill();
    ctx.strokeStyle = '#d8e5f5';
    ctx.stroke();
    ctx.fillStyle = '#4d6b91';
    ctx.font = 'bold 16px Arial';
    ctx.fillText(card.title, card.x + 18, card.y + 28);
    ctx.fillStyle = '#10253f';
    ctx.font = 'bold 18px Arial';
    drawParagraph(ctx, card.value, card.x + 18, card.y + 58, card.w - 36, 24, '#10253f');
  });

  roundedRect(ctx, 60, 1038, 546, 334, 30);
  ctx.fillStyle = '#ffffff';
  ctx.fill();
  ctx.strokeStyle = '#d6e2f2';
  ctx.lineWidth = 2;
  ...
  ctx.fillStyle = '#0d3b78';
  ctx.font = 'bold 24px Arial';
  ctx.fillText('Avant votre venue', 94, 1084);
  const checklist = [
    'Présentez-vous environ 10 minutes avant l’horaire de démarrage indiqué.',
    'Munissez-vous d’une pièce d’identité et du matériel demandé si votre formation le prévoit.',
    'En cas d’empêchement, contactez N.C.R Solutions dès que possible pour réorganiser votre accueil.',
    'Consultez votre espace stagiaire pour retrouver les livrets, supports et documents utiles.'
  ];
  let cy = 1128;
  checklist.forEach(function drawItem(item, index) {
    ctx.fillStyle = '#dbeafe';
    ctx.beginPath();
    ctx.arc(118, cy - 8, 18, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#0d3b78';
    ctx.font = 'bold 16px Arial';
    ctx.fillText(String(index + 1), 113, cy - 2);
    ctx.fillStyle = '#374151';
    ctx.font = '19px Arial';
    cy = drawParagraph(ctx, item, 152, cy, 418, 28, '#374151') + 20;
  });

  roundedRect(ctx, 634, 1038, 546, 334, 30);
  ...
  ctx.fillStyle = '#0d3b78';
  ctx.font = 'bold 24px Arial';
  ctx.fillText('Rappel important', 668, 1084);
  ctx.fillStyle = '#334155';
  ctx.font = '20px Arial';
  drawParagraph(ctx, 'Cette convocation confirme officiellement votre inscription à la session mentionnée ci-dessus.', 668, 1134, 476, 28, '#334155');
  drawParagraph(ctx, 'Conservez ce document. Il peut vous être demandé lors de votre arrivée ou à des fins de justification administrative.', 668, 1214, 476, 28, '#334155');
  drawParagraph(ctx, 'Pour toute question organisationnelle, utilisez prioritairement votre espace stagiaire ou contactez l’organisme via l’adresse renseignée.', 668, 1294, 476, 28, '#334155');

  roundedRect(ctx, 60, 1412, 1120, 180, 30);
  ctx.fillStyle = '#ffffff';
  ctx.fill();
  ctx.strokeStyle = '#d6e2f2';
  ctx.lineWidth = 2;
  ...
  ctx.fillStyle = '#0d3b78';
  ctx.font = 'bold 22px Arial';
  ctx.fillText('Contacts utiles', 92, 1458);
  ctx.fillStyle = '#374151';
  ctx.font = '19px Arial';
  drawParagraph(ctx, `Organisme : N.C.R Solutions • Formateur référent : ${trainerName}`, 92, 1502, 1020, 28, '#374151');
  drawParagraph(ctx, `Email : ${contactEmail} • Espace stagiaire : documents, convocations, attestations et supports sont centralisés dans votre plateforme.`, 92, 1540, 1020, 28, '#374151');

  roundedRect(ctx, 60, 1624, 1120, 74, 24);
  ctx.fillStyle = '#0f172a';
  ctx.fill();
  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 17px Arial';
  ctx.fillText('N.C.R Solutions • Convocation individuelle de formation', 88, 1669);
  ctx.textAlign = 'right';
  ctx.font = '16px Arial';
  ctx.fillText(generatedDate, 1150, 1669);
  ctx.textAlign = 'left';
...
async function createConvocationForStudent(client, trainerSession, sessionRow, studentProfile) {
  ...
  const storagePath = `${studentProfile.id}/convocation/${sessionRow.id}-convocation-${cleanStudentName}-premium-white-${Date.now()}.pdf`;
  ...
  const documentTitle = `Convocation FOND BLANC premium - ${courseTitleForDocuments(sessionRow.course_id)} - ${studentProfile.name || studentProfile.email || 'Stagiaire'}`;
...
async function buildCertificatePdf(session, studentProfile, trainerSession) {
  ...
  ctx.strokeStyle = '#d6e3f4';
  ctx.lineWidth = 2;
  roundedRect(ctx, 44, 44, 1152, 1666, 36);
  ctx.stroke();
  roundedRect(ctx, 58, 58, 1124, 1638, 32);
  ctx.stroke();

  const topGradient = ctx.createLinearGradient(60, 60, 1180, 340);
  topGradient.addColorStop(0, '#0b2f63');
  topGradient.addColorStop(0.6, '#164b8f');
  topGradient.addColorStop(1, '#2b6cb0');
  ctx.fillStyle = topGradient;
  roundedRect(ctx, 84, 84, 1072, 246, 34);
  ctx.fill();
  ...
  if (logo) {
    ...
    const ratio = logo.width / logo.height || 1;
    const targetWidth = Math.min(420, targetHeight * ratio);
    ctx.drawImage(logo, 118, 110, targetWidth, targetHeight);
  }

  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 40px Arial';
  ctx.fillText('N.C.R Solutions', 118, 252);
  ctx.font = '22px Arial';
  ctx.globalAlpha = 0.95;
  ctx.fillText('Formations & Résolutions', 118, 286);
  ctx.globalAlpha = 1;

  ctx.textAlign = 'right';
  ctx.font = 'bold 38px Arial';
  ctx.fillText('Attestation', 1116, 176);
  ctx.font = 'bold 26px Arial';
  ctx.fillText('de fin de stage', 1116, 214);
  ctx.font = '18px Arial';
  ctx.globalAlpha = 0.92;
  ctx.fillText(`Émise le ${generatedDate}`, 1116, 254);
  ctx.globalAlpha = 1;
  ctx.textAlign = 'left';
  ...
  ctx.fillText('ATTESTATION OFFICIELLE DE PARTICIPATION ET DE RÉALISATION', 620, 412);

  ctx.fillStyle = '#0f172a';
  ctx.font = '22px Arial';
  ctx.fillText('La société N.C.R Solutions certifie que', 620, 478);

  roundedRect(ctx, 136, 532, 968, 126, 30);
  ctx.fillStyle = '#ffffff';
  ctx.fill();
  ctx.strokeStyle = '#d6e2f2';
  ctx.lineWidth = 2;
  ...
  ctx.fillText(studentName, 620, 606);
  ctx.fillStyle = '#6b7280';
  ctx.font = '18px Arial';
  ctx.fillText(studentProfile.email || 'Stagiaire', 620, 634);

  ctx.fillStyle = '#1f2937';
  ctx.font = '24px Arial';
  drawParagraph(ctx, `a suivi avec assiduité la formation « ${courseTitle} » organisée ${dateText}.`, 180, 718, 880, 34, '#1f2937');
  drawParagraph(ctx, 'Le parcours de formation a été mené à son terme et validé par le formateur référent.', 180, 772, 880, 34, '#1f2937');

  ctx.textAlign = 'left';
  drawLabeledLines(ctx, 'Identification', [
    `Bénéficiaire : ${studentName}`,
    `Email : ${studentProfile.email || 'Non précisé'}`
  ], { x: 84, y: 860, width: 330, height: 164, fill: '#ffffff', stroke: '#d6e2f2' });

  drawLabeledLines(ctx, 'Formation', [
    `Intitulé : ${courseTitle}`,
    `Session : ${session.title || courseTitle}`
  ], { x: 454, y: 860, width: 330, height: 164, fill: '#ffffff', stroke: '#d6e2f2' });

  drawLabeledLines(ctx, 'Organisation', [
    `Période : ${dateText}`,
    `Formateur : ${trainerName}`
  ], { x: 824, y: 860, width: 332, height: 164, fill: '#ffffff', stroke: '#d6e2f2' });

  roundedRect(ctx, 84, 1064, 1072, 192, 30);
  ctx.fillStyle = '#eef5ff';
  ctx.fill();
  ctx.strokeStyle = '#d6e2f2';
  ctx.lineWidth = 2;
  ...
  ctx.fillStyle = '#0d3b78';
  ctx.font = 'bold 24px Arial';
  ctx.fillText('Portée de l’attestation', 118, 1112);
  ctx.fillStyle = '#334155';
  ctx.font = '20px Arial';
  drawParagraph(ctx, 'Cette attestation confirme la participation du stagiaire et l’achèvement du parcours de formation indiqué. Elle est délivrée à titre de justificatif pédagogique et administratif.', 118, 1154, 1000, 28, '#334155');
  drawParagraph(ctx, 'Elle ne se substitue pas à un certificat réglementaire lorsqu’une procédure légale spécifique est exigée.', 118, 1220, 1000, 28, '#334155');

  ctx.fillStyle = '#ffffff';
  ctx.beginPath();
  ctx.arc(960, 1418, 110, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.textAlign = 'center';
  ctx.font = 'bold 18px Arial';
  ctx.fillText('N.C.R', 960, 1392);
  ctx.fillText('SOLUTIONS', 960, 1418);
  ctx.font = '16px Arial';
  ctx.fillText('Attestation', 960, 1446);
  ctx.fillText('officielle', 960, 1470);
  ...
  ctx.fillStyle = '#6b7280';
  ctx.font = '17px Arial';
  ctx.fillText('Signature / cachet de l’organisme de formation', 112, 1488);
  ctx.fillStyle = '#1f2937';
  ctx.font = 'bold 20px Arial';
  ctx.fillText(trainerName, 112, 1528);
  ctx.font = '18px Arial';
  ctx.fillText('Formateur référent', 112, 1558);

  roundedRect(ctx, 84, 1606, 1072, 64, 22);
  ctx.fillStyle = '#0f172a';
  ctx.fill();
  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 17px Arial';
  ctx.fillText('N.C.R Solutions • Attestation individuelle de fin de stage', 112, 1646);
  ctx.textAlign = 'right';
  ctx.font = '16px Arial';
  ctx.fillText(generatedDate, 1124, 1646);
  ctx.textAlign = 'left';
...
async function createCertificateForStudent(client, trainerSession, sessionRow, studentProfile, sessionStudentRow) {
  const cleanStudentName = sanitizeFileName(studentProfile.name || studentProfile.email || 'stagiaire');
  const storagePath = `${studentProfile.id}/attestation/${sessionRow.id}-attestation-${cleanStudentName}-premium-white-${Date.now()}.pdf`;
  ...
  const documentTitle = `Attestation FOND BLANC premium - ${courseTitleForDocuments(sessionRow.course_id)} - ${studentProfile.name || studentProfile.email || 'Stagiaire'}`;
  const insertResponse = await client
    .from('training_documents')
    .insert({
      title: documentTitle,
