...
async function createConvocationForStudent(client, trainerSession, sessionRow, studentProfile) {
  ...
  const storagePath = `${studentProfile.id}/convocation/${sessionRow.id}-convocation-${cleanStudentName}-ultra-clean-${Date.now()}.pdf`;
  ...
  const documentTitle = `Convocation ULTRA CLEAN - ${courseTitleForDocuments(sessionRow.course_id)} - ${studentProfile.name || studentProfile.email || 'Stagiaire'}`;
...
async function createCertificateForStudent(client, trainerSession, sessionRow, studentProfile, sessionStudentRow) {
  const cleanStudentName = sanitizeFileName(studentProfile.name || studentProfile.email || 'stagiaire');
  const storagePath = `${studentProfile.id}/attestation/${sessionRow.id}-attestation-${cleanStudentName}-ultra-clean-${Date.now()}.pdf`;
  ...
  const documentTitle = `Attestation ULTRA CLEAN - ${courseTitleForDocuments(sessionRow.course_id)} - ${studentProfile.name || studentProfile.email || 'Stagiaire'}`;
  const insertResponse = await client
    .from('training_documents')
    .insert({
      title: documentTitle,
