...
async function createConvocationForStudent(client, trainerSession, sessionRow, studentProfile) {
  ...
  const documentTitle = `Convocation premium - ${courseTitleForDocuments(sessionRow.course_id)} - ${studentProfile.name || studentProfile.email || 'Stagiaire'}`;
...
async function createCertificateForStudent(client, trainerSession, sessionRow, studentProfile, sessionStudentRow) {
  ...
  const documentTitle = `Attestation premium de fin de stage - ${courseTitleForDocuments(sessionRow.course_id)} - ${studentProfile.name || studentProfile.email || 'Stagiaire'}`;
  const insertResponse = await client
    .from('training_documents')
    .insert({
      title: documentTitle,
