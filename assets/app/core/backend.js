      ...
      from(bucket) {
        return {
          ...
          async remove(paths) {
            ...
            const response = await fetch(`${client.url}/storage/v1/object/${bucket}`, {
              method: 'DELETE',
              headers: {
                apikey: client.key,
                Authorization: `Bearer ${token || client.key}`,
...
export async function createRemoteStudent(session, payload) {
  ...
  try {
    await sendRemoteEmailNotification(session, {
      type: 'welcome',
      toEmail: email,
      toName: fullName,
      courses: courseIds.map(function mapCourse(courseId) { return courseTitleForDocuments(courseId); })
    });
  } catch (emailError) {
    warnEmailError('bienvenue stagiaire', emailError);
  }
...
export async function listRemoteDocuments(session) {
  ...
  const response = await sdk.client
    ...
    .select('id, title, name, type, course_id, target_user_id, session_id, bucket, storage_path, status, visibility, created_at')
    .eq('status', 'active')
    .order('created_at', { ascending: false });
  if (response.error) throw response.error;
...
export async function deleteRemoteDocument(session, documentId) {
  if (!hasValidConfig() || !session || session.role !== 'trainer') throw new Error('Suppression réservée au formateur connecté.');
  const sdk = await loadSupabase();
  const docResponse = await sdk.client
    .from('training_documents')
    .select('id, bucket, storage_path')
    .eq('id', documentId)
    .single();
  if (docResponse.error) throw new Error(`Document introuvable : ${readableSupabaseError(docResponse.error)}`);

  const document = docResponse.data;
  if (document && document.bucket && document.storage_path) {
    try {
      const removeStorage = await sdk.client.storage.from(document.bucket).remove([document.storage_path]);
      if (removeStorage && removeStorage.error) {
        console.warn('Fichier Storage non supprimé, suppression de la ligne document poursuivie.', removeStorage.error);
      }
    } catch (storageError) {
      console.warn('Suppression Storage indisponible, suppression de la ligne document poursuivie.', storageError);
    }
  }

  const deleteResponse = await sdk.client
    .from('training_documents')
    .delete()
    .eq('id', documentId);
  if (deleteResponse.error) throw new Error(`Suppression document : ${readableSupabaseError(deleteResponse.error)}`);
  return true;
}

export async function listRemoteSatisfaction(session) {
  if (!hasValidConfig() || !session || !session.id) return [];
  const sdk = await loadSupabase();
  const response = await sdk.client
    .from('satisfaction_surveys')
    .select('id, student_id, student_name, student_email, course_id, rating, useful, improvement, comment, created_at')
    .order('created_at', { ascending: false });
  if (response.error) throw response.error;
  return (response.data || []).map(function normalizeSatisfaction(item) {
    return {
      id: item.id,
      studentId: item.student_id,
      studentName: item.student_name || 'Stagiaire',
...
export async function createRemoteSatisfaction(session, payload) {
  if (!hasValidConfig() || !session || session.role !== 'student') throw new Error('Questionnaire réservé au stagiaire connecté.');
  const sdk = await loadSupabase();
  const rating = Number(payload && payload.rating || 0);
  if (!rating || rating < 1 || rating > 5) throw new Error('Choisis une note entre 1 et 5.');
  const response = await sdk.client
    .from('satisfaction_surveys')
    .insert({
      student_id: session.id,
      student_name: session.name || session.email || 'Stagiaire',
      student_email: session.email || '',
      course_id: String(payload && payload.courseId || ''),
      rating,
      useful: String(payload && payload.useful || '').trim(),
      improvement: String(payload && payload.improvement || '').trim(),
      comment: String(payload && payload.comment || '').trim(),
      created_at: new Date().toISOString()
    })
    .select()
    .single();
  if (response.error) throw new Error(`Questionnaire satisfaction : ${readableSupabaseError(response.error)}`);
  return response.data;
}
...
function emailAppUrl() {
  try {
    ...
    return 'https://sst-maker.github.io/NCR-ACADEMY/#/login';
  }
}

function compactDateRange(startDate, endDate) {
  const start = formatDocumentDate(startDate);
  const end = formatDocumentDate(endDate);
  return start === end ? start : `${start} au ${end}`;
}

function compactHours(startTime, endTime) {
  const start = formatDocumentTime(startTime);
  const end = formatDocumentTime(endTime);
  if (!start && !end) return 'Horaires à confirmer';
  return `${start || 'heure à confirmer'} - ${end || 'heure à confirmer'}`;
}

async function sendRemoteEmailNotification(session, payload) {
  if (!hasValidConfig() || !session || session.role !== 'trainer') return { skipped: true };
  const sdk = await loadSupabase();
  const token = sdk.client.getAccessToken ? sdk.client.getAccessToken() : '';
  if (!token) throw new Error('Session Supabase expirée : reconnecte-toi pour envoyer les emails automatiques.');

  const response = await fetch(`${sdk.client.url}/functions/v1/ncr-send-email`, {
    method: 'POST',
    headers: {
      apikey: sdk.client.key,
    ...
    body: JSON.stringify(Object.assign({
      appUrl: emailAppUrl(),
      trainerName: session.name || session.email || 'N.C.R Solutions',
  ...
  if (!response.ok) {
    const message = data && (data.error || data.message) ? (data.error || data.message) : raw || response.statusText;
    throw new Error(`Email automatique : ${message}`);
  }
  return data || { ok: true };
}

function warnEmailError(context, error) {
  console.warn(`Email automatique non envoyé (${context}).`, error && error.message ? error.message : error);
}
...
function buildPdfFromJpegPages(pages) {
  const safePages = Array.isArray(pages) ? pages.filter(function validPage(page) {
    return page && page.jpegBytes && page.jpegBytes.length && page.width && page.height;
  }) : [];
  if (!safePages.length) throw new Error('Aucune page PDF à générer.');
  if (safePages.length === 1) {
    const first = safePages[0];
    return buildPdfFromJpegBytes(first.jpegBytes, first.width, first.height);
  }
  ...
  const pageHeight = 841.89;
  const objects = [];
  const pageObjectIds = safePages.map(function pageId(_, index) { return 3 + index * 3; });

  objects.push(encoder.encode('1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n'));
  objects.push(encoder.encode(`2 0 obj\n<< /Type /Pages /Kids [${pageObjectIds.map(function ref(id) { return `${id} 0 R`; }).join(' ')}] /Count ${safePages.length} >>\nendobj\n`));

  safePages.forEach(function appendPage(page, index) {
    ...
    const content = `q\n${drawWidth.toFixed(2)} 0 0 ${drawHeight.toFixed(2)} ${offsetX.toFixed(2)} ${offsetY.toFixed(2)} cm\n/Im${index} Do\nQ`;
    const contentBytes = encoder.encode(content);
    objects.push(encoder.encode(`${pageId} 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595.28 841.89] /Resources << /ProcSet [/PDF /ImageC] /XObject << /Im${index} ${imageId} 0 R >> >> /Contents ${contentId} 0 R >>\nendobj\n`));
    objects.push(encoder.encode(`${contentId} 0 obj\n<< /Length ${contentBytes.length} >>\nstream\n${content}\nendstream\nendobj\n`));
...
function attendancePeriodName(period) {
  return period === 'afternoon' ? 'Après-midi' : 'Matin';
}

function attendanceStatusName(status) {
  if (status === 'signed') return 'Signé';
  if (status === 'absent') return 'Absent';
  if (status === 'refused') return 'Refus de signer';
...
export async function listRemoteAttendanceSignatures(session) {
  if (!hasValidConfig() || !session || session.role !== 'trainer') return [];
  const sdk = await loadSupabase();
  const response = await sdk.client
    .from('attendance_signatures')
    .select('id, session_id, attendance_date, period, participant_id, participant_role, participant_name, status, signature_data, signed_at, collected_by, note, created_at, updated_at')
    .order('signed_at', { ascending: false });
  if (response.error) throw new Error(`Émargements : ${readableSupabaseError(response.error)}`);
  return (response.data || []).map(function normalizeAttendance(row) {
    return {
      id: row.id,
...
export async function upsertRemoteAttendanceSignature(session, payload) {
  if (!hasValidConfig() || !session || session.role !== 'trainer') throw new Error('Émargement réservé au formateur connecté.');
  const sdk = await loadSupabase();
  const status = String(payload && payload.status || '');
  const role = String(payload && payload.participantRole || 'student');
  const period = String(payload && payload.period || 'morning');
  if (['signed', 'absent', 'refused'].indexOf(status) === -1) throw new Error('Statut d’émargement invalide.');
  if (['student', 'trainer'].indexOf(role) === -1) throw new Error('Rôle de signature invalide.');
  if (['morning', 'afternoon'].indexOf(period) === -1) throw new Error('Créneau invalide.');
  if (role === 'trainer' && status !== 'signed') throw new Error('La validation formateur doit comporter une signature.');
  if (status === 'signed' && !String(payload && payload.signatureData || '').startsWith('data:image/')) {
    throw new Error('La signature est vide.');
  }

  const now = new Date().toISOString();
  const row = {
    session_id: String(payload && payload.sessionId || ''),
    attendance_date: String(payload && payload.attendanceDate || ''),
    period,
    participant_id: String(payload && payload.participantId || ''),
    participant_role: role,
    participant_name: String(payload && payload.participantName || '').trim() || (role === 'trainer' ? 'Formateur' : 'Stagiaire'),
    status,
    signature_data: status === 'signed' ? String(payload && payload.signatureData || '') : null,
    signed_at: now,
    collected_by: session.id,
    note: String(payload && payload.note || '').trim() || null,
    updated_at: now
  };
  if (!row.session_id || !row.attendance_date || !row.participant_id) throw new Error('Informations d’émargement incomplètes.');

  const response = await sdk.client
    .from('attendance_signatures')
    .upsert(row, { onConflict: 'session_id,attendance_date,period,participant_id' })
    .select()
    .single();
  if (response.error) throw new Error(`Enregistrement de la signature : ${readableSupabaseError(response.error)}`);

  if (role === 'student') {
    const attendanceStatus = status === 'absent' ? 'absent' : 'present';
    const studentUpdate = await sdk.client
      .from('session_students')
      .update({ attendance_status: attendanceStatus })
      .eq('session_id', row.session_id)
      .eq('student_id', row.participant_id);
    if (studentUpdate.error) throw new Error(`Synchronisation de la présence : ${readableSupabaseError(studentUpdate.error)}`);
  }

  return response.data;
}

async function attendanceRecordsForSlot(client, sessionId, attendanceDate, period) {
  ...
  if (response.error) throw new Error(`Signatures : ${readableSupabaseError(response.error)}`);
  return response.data || [];
}

async function buildSignedAttendanceSheetPdf(sessionRow, students, records, trainerSession, attendanceDate, period) {
  const periodLabel = attendancePeriodName(period);
  const courseTitle = courseTitleForDocuments(sessionRow.course_id);
  const generatedDate = new Intl.DateTimeFormat('fr-FR', { dateStyle: 'long' }).format(new Date());
  const recordByParticipant = new Map(records.map(function mapRecord(record) { return [record.participant_id, record]; }));
  const trainerRecord = records.find(function trainerSignature(record) { return record.participant_role === 'trainer'; });
  const chunkSize = 10;
  const chunks = [];
  const sourceStudents = students && students.length ? students : [];
  for (let index = 0; index < Math.max(sourceStudents.length, 1); index += chunkSize) {
    chunks.push(sourceStudents.slice(index, index + chunkSize));
  }
  if (!chunks.length) chunks.push([]);

  const pages = [];
  const logo = await loadDocumentLogo();
  const trainerImage = trainerRecord && trainerRecord.signature_data ? await loadDataImage(trainerRecord.signature_data) : null;

  for (let pageIndex = 0; pageIndex < chunks.length; pageIndex += 1) {
    const pageStudents = chunks[pageIndex];
    const canvas = document.createElement('canvas');
    canvas.width = 1240;
    canvas.height = 1754;
    const ctx = canvas.getContext('2d');
    ...
    gradient.addColorStop(1, '#24518e');
    ctx.fillStyle = gradient;
    roundedRect(ctx, 60, 54, 1120, 245, 38);
    ctx.fill();

    if (logo) {
      const targetHeight = 86;
      const ratio = logo.width / logo.height || 1;
      ctx.drawImage(logo, 86, 92, Math.min(360, targetHeight * ratio), targetHeight);
    }
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 34px Arial';
    ctx.fillText('N.C.R Solutions', 86, 222);
    ctx.font = '18px Arial';
    ctx.fillText('Parler vrai, former juste.', 86, 258);
    ctx.textAlign = 'right';
    ctx.font = 'bold 40px Arial';
    ctx.fillText('Feuille d’émargement signée', 1136, 132);
    ctx.font = 'bold 23px Arial';
    ctx.fillText(`${periodLabel} · ${formatDocumentDate(attendanceDate)}`, 1136, 176);
    ctx.font = '17px Arial';
    ctx.fillText(`Page ${pageIndex + 1}/${chunks.length} · Générée le ${generatedDate}`, 1136, 218);
    ctx.textAlign = 'left';

    roundedRect(ctx, 80, 340, 1080, 190, 28);
    ctx.fillStyle = '#ffffff';
    ctx.fill();
    ctx.strokeStyle = '#d6e2f2';
    ctx.lineWidth = 2;
    ...
    ctx.fillText('Informations de session', 112, 393);
    ctx.fillStyle = '#111827';
    ctx.font = '19px Arial';
    ctx.fillText(`Formation : ${courseTitle}`, 112, 440);
    ctx.fillText(`Session : ${sessionRow.title || courseTitle}`, 112, 480);
    ctx.fillText(`Lieu : ${sessionRow.location || 'Non précisé'}`, 650, 440);
    ctx.fillText(`Formateur : ${trainerSession.name || trainerSession.email || 'N.C.R Solutions'}`, 650, 480);
    ...
    const rowH = 72;
    const colNameW = 360;
    const colStatusW = 190;
    const colSignatureW = tableW - colNameW - colStatusW;
    ...
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 18px Arial';
    ctx.fillText('Stagiaire', tableX + 20, tableY + 37);
    ctx.fillText('Statut / heure', tableX + colNameW + 20, tableY + 37);
    ctx.fillText('Signature', tableX + colNameW + colStatusW + 20, tableY + 37);
    ...
    if (!pageStudents.length) {
      ...
      ctx.fillStyle = '#6b7280';
      ctx.font = '18px Arial';
      ctx.fillText('Aucun stagiaire rattaché à cette session.', tableX + 20, tableY + headerH + 43);
    }

    for (let rowIndex = 0; rowIndex < pageStudents.length; rowIndex += 1) {
      const student = pageStudents[rowIndex];
      const profile = student.profile || {};
      const record = recordByParticipant.get(student.student_id);
      const y = tableY + headerH + rowIndex * rowH;
      ctx.fillStyle = rowIndex % 2 === 0 ? '#ffffff' : '#f8fbff';
      ctx.fillRect(tableX, y, tableW, rowH);
      ctx.strokeStyle = '#d6e2f2';
      ctx.strokeRect(tableX, y, tableW, rowH);
      ctx.beginPath(); ctx.moveTo(tableX + colNameW, y); ctx.lineTo(tableX + colNameW, y + rowH); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(tableX + colNameW + colStatusW, y); ctx.lineTo(tableX + colNameW + colStatusW, y + rowH); ctx.stroke();

      ctx.fillStyle = '#111827';
      ctx.font = 'bold 17px Arial';
      ctx.fillText(profile.name || 'Stagiaire', tableX + 18, y + 29);
      ctx.fillStyle = '#6b7280';
      ctx.font = '14px Arial';
      ctx.fillText(profile.email || '', tableX + 18, y + 52);

      const status = record ? record.status : '';
      ctx.fillStyle = status === 'signed' ? '#16794a' : status === 'absent' ? '#b42318' : status === 'refused' ? '#9a6700' : '#6b7280';
      ctx.font = 'bold 16px Arial';
      ctx.fillText(attendanceStatusName(status), tableX + colNameW + 18, y + 29);
      ctx.fillStyle = '#6b7280';
      ctx.font = '14px Arial';
      ctx.fillText(record ? attendanceTime(record.signed_at) : '', tableX + colNameW + 18, y + 52);

      if (record && record.status === 'signed' && record.signature_data) {
        const signatureImage = await loadDataImage(record.signature_data);
        drawContainedImage(ctx, signatureImage, tableX + colNameW + colStatusW + 16, y + 7, colSignatureW - 32, rowH - 14);
      } else if (record && record.status !== 'signed') {
        ctx.fillStyle = '#6b7280';
        ctx.font = 'italic 15px Arial';
        ctx.fillText(attendanceStatusName(record.status), tableX + colNameW + colStatusW + 22, y + 42);
      }
    }

    const trainerY = 1420;
    roundedRect(ctx, 80, trainerY, 1080, 180, 26);
    ctx.fillStyle = '#eef5ff';
    ctx.fill();
    ctx.strokeStyle = '#d6e2f2';
    ctx.stroke();
    ctx.fillStyle = '#0d3b78';
    ctx.font = 'bold 22px Arial';
    ctx.fillText('Validation du formateur', 112, trainerY + 45);
    ctx.fillStyle = '#374151';
    ctx.font = '17px Arial';
    ctx.fillText(trainerRecord ? `${trainerRecord.participant_name || trainerSession.name || 'Formateur'} · signé à ${attendanceTime(trainerRecord.signed_at)}` : 'Signature formateur manquante', 112, trainerY + 83);
    if (trainerImage) {
      drawContainedImage(ctx, trainerImage, 720, trainerY + 20, 390, 135);
    }

    roundedRect(ctx, 60, 1640, 1120, 70, 22);
    ctx.fillStyle = '#0f172a';
    ctx.fill();
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 15px Arial';
    ctx.fillText('N.C.R Solutions', 88, 1682);
    ctx.font = '14px Arial';
    ctx.fillText('Émargement numérique collecté sur l’appareil du formateur', 260, 1682);
    ctx.textAlign = 'right';
    ctx.fillText(`${periodLabel} · ${formatDocumentDate(attendanceDate)}`, 1148, 1682);
    ctx.textAlign = 'left';

    const jpegDataUrl = canvas.toDataURL('image/jpeg', 0.93);
    pages.push({ jpegBytes: dataUrlToUint8Array(jpegDataUrl), width: canvas.width, height: canvas.height });
  }

  return buildPdfFromJpegPages(pages);
}

export async function generateRemoteSignedAttendanceSheet(session, sessionId, attendanceDate, period) {
  if (!hasValidConfig() || !session || session.role !== 'trainer') throw new Error('Génération réservée au formateur connecté.');
  const sdk = await loadSupabase();
  const data = await fetchSessionWithStudents(sdk.client, sessionId);
  const sessionRow = data.session;
  const records = await attendanceRecordsForSlot(sdk.client, sessionId, attendanceDate, period);
  const studentRecords = new Map(records.filter(function studentOnly(record) { return record.participant_role === 'student'; }).map(function pair(record) { return [record.participant_id, record]; }));
  const missing = data.students.filter(function missingStudent(student) { return !studentRecords.has(student.student_id); });
  const trainerRecord = records.find(function trainerOnly(record) { return record.participant_role === 'trainer' && record.status === 'signed' && record.signature_data; });
  if (missing.length) throw new Error(`${missing.length} stagiaire(s) n’ont pas encore de statut pour ce créneau.`);
  if (!trainerRecord) throw new Error('La signature finale du formateur est obligatoire avant de générer le PDF.');

  const pdf = await buildSignedAttendanceSheetPdf(sessionRow, data.students, records, session, attendanceDate, period);
  const periodLabel = attendancePeriodName(period);
  const safeTitle = sanitizeFileName(sessionRow.title || sessionRow.id || 'session');
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  const dateSlug = String(attendanceDate || '').replace(/[^0-9-]/g, '') || 'date';
  const storagePath = `formateur/emargements/${sessionRow.id}/${dateSlug}-${period}-${stamp}-emargement-signe-${safeTitle}.pdf`;
  const uploadResponse = await sdk.client.storage
    .from('documents-prives')
    .upload(storagePath, pdf, { cacheControl: '3600', upsert: false, contentType: 'application/pdf' });
  if (uploadResponse.error) throw new Error(`Upload émargement signé : ${readableSupabaseError(uploadResponse.error)}`);

  const documentTitle = `Émargement signé ${periodLabel} ${formatDocumentDate(attendanceDate)} - ${sessionRow.title || courseTitleForDocuments(sessionRow.course_id)}`;
  const insertResponse = await sdk.client
    .from('training_documents')
    .insert({
      title: documentTitle,
    ...
    .select()
    .single();
  if (insertResponse.error) throw new Error(`Fiche document émargement signé : ${readableSupabaseError(insertResponse.error)}`);
  return insertResponse.data;
}
    ...
    for (const row of eligibleRows) {
      ...
      try {
        await sendRemoteEmailNotification(session, {
          ...
          toName: profile.name || profile.email || 'Stagiaire',
          courseTitle: courseTitleForDocuments(sessionRow.course_id),
          sessionTitle: sessionRow.title || courseTitleForDocuments(sessionRow.course_id),
          dateText: compactDateRange(sessionRow.start_date, sessionRow.end_date),
          hoursText: compactHours(sessionRow.start_time, sessionRow.end_time),
          location: sessionRow.location || 'Lieu à confirmer'
        });
      } catch (emailError) {
        warnEmailError(`attestation ${profile.email || profile.name || ''}`, emailError);
      }
    }
  }
  ...
  for (const studentProfile of studentProfiles) {
    ...
    try {
      await sendRemoteEmailNotification(session, {
        ...
        toName: studentProfile.name || studentProfile.email || 'Stagiaire',
        courseTitle: courseTitleForDocuments(created.course_id),
        sessionTitle: created.title || courseTitleForDocuments(created.course_id),
        dateText: compactDateRange(created.start_date, created.end_date),
        hoursText: compactHours(created.start_time, created.end_time),
        location: created.location || 'Lieu à confirmer'
      });
    } catch (emailError) {
      warnEmailError(`convocation ${studentProfile.email || studentProfile.name || ''}`, emailError);
    }
  }
