...
function buildSimplePdf(title, lines) {
  const safeTitle = pdfText(title || 'Document de formation');
  const contentLines = [];
  contentLines.push('BT');
  contentLines.push('/F1 19 Tf');
  contentLines.push('50 790 Td');
  contentLines.push(`(${safeTitle}) Tj`);
  contentLines.push('0 -28 Td');
  contentLines.push('/F1 10.5 Tf');
  (lines || []).forEach(function addLine(line) {
    wrapPdfLine(line, 86).forEach(function addWrapped(wrapped) {
      contentLines.push(`(${pdfText(wrapped)}) Tj`);
  ...
  let pdf = '%PDF-1.4\n';
  const offsets = [0];
  objects.forEach(function appendObject(object) {
...
function buildConvocationPdf(session, studentProfile, trainerSession) {
  ...
  const schedule = startTime || endTime ? `${startTime || 'heure a confirmer'} - ${endTime || 'heure a confirmer'}` : 'Horaires a confirmer';
  const generatedDate = new Intl.DateTimeFormat('fr-FR').format(new Date());
  const title = 'Convocation a une session de formation';
  const lines = [
    'N.C.R Academy - N.C.R Solutions',
    '',
    `Stagiaire : ${studentProfile.name || studentProfile.email || 'Stagiaire'}`,
    `Email : ${studentProfile.email || 'Non precise'}`,
    `Formation : ${courseTitle}`,
    `Session : ${session.title || courseTitle}`,
    `Dates : du ${formatDocumentDate(session.start_date)} au ${formatDocumentDate(session.end_date)}`,
    `Horaires : ${schedule}`,
    `Lieu : ${session.location || 'Lieu a confirmer'}`,
    `Formateur responsable : ${trainerSession.name || trainerSession.email || 'N.C.R Solutions'}`,
    '',
    'Vous etes convoque a la session de formation indiquee ci-dessus. Merci de vous presenter a l heure prevue avec une piece d identite et le materiel utile a la formation si necessaire.',
    '',
    'Les supports et documents lies a cette formation sont consultables dans votre espace stagiaire N.C.R Academy.',
    '',
    'Document genere automatiquement par N.C.R Academy.',
...
async function createConvocationForStudent(client, trainerSession, sessionRow, studentProfile) {
  ...
  const pdf = buildConvocationPdf(sessionRow, studentProfile, trainerSession);
