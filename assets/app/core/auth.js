...
const SESSION_KEY = 'ncr_academy_session_v4_operationnelle';
...
export async function login(email, password) {
  ...
  if (isBackendEnabled()) {
    try {
      ...
      if (!allowDemoFallback()) throw new Error('Connexion Firebase refusée : vérifie le compte, le mot de passe ou la configuration du projet.');
      session = localLogin(email, password);
      session.backendWarning = 'Firebase configuré mais connexion refusée : fallback démo activé.';
    }
  } else {
    session = localLogin(email, password);
  }
