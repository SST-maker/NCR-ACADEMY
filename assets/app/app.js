...
import { isBackendEnabled, getBackendDiagnosticSnapshot, testBackendConnection, loadRemoteState, saveRemoteState, recordRemoteEvent, createRemoteTicket, listRemoteDocuments, listRemoteUsers, uploadRemoteDocument } from './core/backend.js';
import { courses, getCourse } from './data/catalog.js';
import { qualiopiChecklist, documentTemplates } from './data/qualopi.js';

const app = document.querySelector('#app');
const ASSET_VERSION = '5.3.1';
...
function documentsView(role) {
  ...
  const allDocs = documentTemplates.concat(supabaseDocs);
  ...
  return appShell(template`
    <section class="glass-panel content-card fade-in">
      ...
      <p class="lead small">Les modèles inclus restent disponibles hors ligne. Les documents privés ajoutés via Supabase apparaissent ici quand le compte est connecté.</p>
      ${emptyRemoteNotice}
      ${trainerUpload}
      <div class="document-grid">
        ${allDocs.map(function renderDoc(doc) {
...
function trainerSessionsView() {
  ...
  return appShell(template`
    <section class="hero-card glass-panel fade-in">
      ...
      <p>Prépare une session, rattache les stagiaires, puis active ensuite la génération automatique des convocations et attestations.</p>
    </section>
    ...
    <section class="section-grid two fade-in delay-1">
      <article class="glass-panel content-card">
        ...
        <form id="session-create-form" class="auth-form compact-form">
          ...
          <button class="primary-button" type="submit">Préparer la session</button>
        </form>
      </article>

      <article class="glass-panel content-card accent-card">
        <div class="section-heading"><p class="eyebrow">Automatisation prévue</p><h2>Flux documents</h2></div>
        <ul class="check-list clean">
          <li><strong>Création session</strong><span>Enregistrement de la formation, du lieu, des dates et du formateur.</span></li>
          <li><strong>Ajout stagiaires</strong><span>Association des élèves à la session et à leur parcours.</span></li>
          <li><strong>Convocations</strong><span>Génération PDF automatique par stagiaire après validation de la session.</span></li>
          <li><strong>Attestations</strong><span>Génération uniquement à la clôture, après présence et réussite validées.</span></li>
        </ul>
      </article>
    </section>
  `, 'trainer');
...
function bindEvents() {
  ...
  if (sessionCreateForm) sessionCreateForm.addEventListener('submit', function submitSessionDraft(event) {
    event.preventDefault();
    const notice = sessionCreateForm.querySelector('.form-error');
    if (notice) {
      notice.textContent = 'Interface Sessions ajoutée. Prochaine étape : lancer le SQL Supabase pour enregistrer les sessions, puis brancher la génération automatique des PDF.';
      notice.hidden = false;
    }
    trackEvent('session_form_prepared', { source: 'trainer_sessions_view' });
  });
