...
import { isBackendEnabled, loadRemoteState, saveRemoteState, recordRemoteEvent, createRemoteTicket, listRemoteDocuments } from './core/backend.js';
...
function isFirebaseSession() {
  return Boolean(session && session.backend === 'firebase' && isBackendEnabled());
}

async function hydrateRemoteData() {
  if (!isFirebaseSession() || remoteHydratedFor === session.id) return;
  try {
  ...
  return template`
    <main class="login-page">
      <section class="login-card glass-panel" aria-labelledby="login-title">
        <div class="login-brand"><img src="./assets/brand/logo-ncr-academy.png" alt="Logo N.C.R Solutions" /></div>
...
function documentsView(role) {
  const firebaseDocs = remoteDocuments.map(function normalizeRemoteDoc(doc) {
    return {
      name: doc.name || doc.title || 'Document distant',
      type: doc.type || 'Document',
      status: doc.status || 'Disponible en ligne',
      href: doc.downloadUrl || doc.href || ''
    };
  });
  const allDocs = documentTemplates.concat(firebaseDocs);
  return appShell(template`
    <section class="glass-panel content-card fade-in">
      ...
      <p class="lead small">Les modèles inclus restent disponibles hors ligne. Les documents ajoutés via Firebase apparaissent ici quand le compte est connecté.</p>
      <div class="document-grid">
        ${allDocs.map(function renderDoc(doc) { return `<article><span>${escapeHTML(doc.type)}</span><strong>${escapeHTML(doc.name)}</strong><p>${escapeHTML(doc.status)}</p>${doc.href ? `<a class="ghost-button doc-link" href="${escapeHTML(doc.href)}" target="_blank" rel="noopener">Ouvrir</a>` : '<button class="ghost-button" disabled>À connecter</button>'}</article>`; }).join('')}
      </div>
    </section>
  `, role || 'student');
}
  ...
  return appShell(template`
    <section class="section-grid two fade-in">
      <article class="glass-panel content-card">
        ...
        <ul class="soft-list">
          <li>Remplacer l’authentification de démonstration par Firebase Auth ou Supabase Auth.</li>
          <li>Brancher le stockage documents côté serveur pour les convocations, attestations et livrets.</li>
...
function bindEvents() {
  ...
  if (ticketForm) ticketForm.addEventListener('submit', function submitTicket(event) {
    ...
    if (isFirebaseSession()) createRemoteTicket(session, ticket).catch(function ignoreTicketError(error) { console.warn('Ticket distant indisponible', error); });
