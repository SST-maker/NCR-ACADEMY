...
let remoteAttendanceSignatures = [];
let attendanceUiState = { sessionId: '', attendanceDate: '', period: 'morning', signingParticipantId: '', signingParticipantRole: '', notice: '' };
let remoteHydratedFor = null;
let documentLibraryState = {
  view: 'course',
  courseId: '',
  categoryId: '',
  studentId: '',
...
async function hydrateRemoteData() {
  ...
  try {
    ...
    remoteAttendanceSignatures = session.role === 'trainer' ? await listRemoteAttendanceSignatures(session) : [];
...
function trainerDrawerLinks(current) {
  const links = [
    ...
    ['/trainer/attendance', 'Émargement numérique'],
...
const DOCUMENT_CATEGORIES = [
  { id: 'convocation', label: 'Convocations', short: 'CV', description: 'Informations officielles avant la formation.' },
  { id: 'livret', label: 'Livrets pédagogiques', short: 'LP', description: 'Livrets et ressources de parcours.' },
  { id: 'support', label: 'Supports pédagogiques', short: 'SP', description: 'Cours, présentations et supports de formation.' },
  { id: 'workbook', label: 'Cahiers d’exercices', short: 'EX', description: 'Ateliers, activités et mises en pratique.' },
  { id: 'glossary', label: 'Glossaires', short: 'GL', description: 'Définitions et vocabulaire professionnel.' },
  { id: 'memo', label: 'Fiches mémo', short: 'FM', description: 'Synthèses et fiches réflexes.' },
  { id: 'attendance', label: 'Émargements', short: 'EM', description: 'Feuilles de présence liées aux sessions.' },
  { id: 'certificate', label: 'Attestations', short: 'AT', description: 'Attestations et justificatifs de fin de formation.' },
  { id: 'other', label: 'Autres documents', short: 'AD', description: 'Documents complémentaires non classés.' }
];
...
function documentCategoryId(doc) {
  const type = normalizeDocumentType(doc && doc.type);
  const name = String(doc && (doc.name || doc.title) || '').toLowerCase();
  if (type === 'convocation' || name.indexOf('convocation') !== -1) return 'convocation';
  if (type === 'livret' || type === 'livret-pedagogique' || name.indexOf('livret') !== -1) return 'livret';
  if (['support', 'support-pedagogique', 'cours', 'presentation'].indexOf(type) !== -1 || name.indexOf('support') !== -1) return 'support';
  if (['workbook', 'cahier', 'cahier-exercices', 'exercice', 'exercises'].indexOf(type) !== -1 || name.indexOf('cahier') !== -1 || name.indexOf('exercice') !== -1) return 'workbook';
  if (['glossary', 'glossaire'].indexOf(type) !== -1 || name.indexOf('glossaire') !== -1) return 'glossary';
  if (['memo', 'fiche-memo', 'fiche-reflexe'].indexOf(type) !== -1 || name.indexOf('mémo') !== -1 || name.indexOf('memo') !== -1 || name.indexOf('fiche réflexe') !== -1) return 'memo';
  if (['emargement', 'attendance', 'feuille-emargement'].indexOf(type) !== -1 || name.indexOf('émargement') !== -1 || name.indexOf('emargement') !== -1) return 'attendance';
  if (['attestation', 'certificate', 'certificat'].indexOf(type) !== -1 || name.indexOf('attestation') !== -1 || name.indexOf('certificat') !== -1) return 'certificate';
  return 'other';
}

function documentCategory(categoryId) {
  return DOCUMENT_CATEGORIES.find(function findCategory(item) { return item.id === categoryId; }) || DOCUMENT_CATEGORIES[DOCUMENT_CATEGORIES.length - 1];
}
...
function normalizeLibraryDocument(doc) {
  const normalized = {
    ...
    courseId: doc.course_id || doc.courseId || '',
    targetUserId: doc.target_user_id || doc.targetUserId || '',
    sessionId: doc.session_id || doc.sessionId || '',
    visibility: doc.visibility || 'course',
    createdAt: doc.created_at || doc.createdAt || ''
  };
  normalized.categoryId = documentCategoryId(normalized);
  return normalized;
}
...
function courseFolderShort(courseId) {
  const labels = {
    'communication-digitale': 'COM',
    'outils-bureautiques': 'BUR',
    'litiges-clients': 'LIT',
    'community-manager': 'CM',
    'acteur-sst': 'SST',
    '__unclassified__': 'DOC'
  };
  return labels[courseId] || 'NCR';
}

function courseFolderTitle(courseId) {
  return courseId === '__unclassified__' ? 'Documents non classés' : courseTitle(courseId);
}

function documentsForCourse(documents, courseId) {
  if (courseId === '__unclassified__') return documents.filter(function noCourse(doc) { return !doc.courseId; });
  return documents.filter(function sameCourse(doc) { return doc.courseId === courseId; });
}

function documentsForStudent(documents, student) {
  if (!student) return [];
  const allowedCourses = new Set(Array.isArray(student.courses) ? student.courses : []);
  return documents.filter(function belongsToStudent(doc) {
    if (doc.targetUserId === student.id) return true;
    return !doc.targetUserId && doc.courseId && allowedCourses.has(doc.courseId);
  });
}

function courseIdsForDocuments(documents, preferredIds) {
  const ids = uniqueValues((preferredIds || []).concat(documents.map(function mapCourse(doc) { return doc.courseId; })));
  if (documents.some(function hasUnclassified(doc) { return !doc.courseId; })) ids.push('__unclassified__');
  return uniqueValues(ids);
}

function renderDocumentViewTabs(activeView) {
  const tabs = [
    ['course', 'Par formation'],
    ['student', 'Par stagiaire'],
    ['session', 'Par session']
  ];
  return `<div class="document-view-tabs" role="tablist" aria-label="Classement des documents">${tabs.map(function tab(item) {
    return `<button class="document-view-tab ${activeView === item[0] ? 'active' : ''}" type="button" data-doc-action="set-view" data-view="${item[0]}">${item[1]}</button>`;
  }).join('')}</div>`;
}

function renderCourseFolders(documents, courseIds, contextLabel) {
  if (!courseIds.length) return '<div class="document-empty"><strong>Aucune formation à afficher</strong><span>Les dossiers apparaîtront automatiquement dès qu’un document sera associé à une formation.</span></div>';
  return `<div class="document-folder-grid">${courseIds.map(function courseFolder(courseId) {
    const folderDocs = documentsForCourse(documents, courseId);
    const categoryCount = new Set(folderDocs.map(function category(doc) { return doc.categoryId; })).size;
    return `<button class="document-folder-card" type="button" data-doc-action="open-course" data-course-id="${escapeHTML(courseId)}">
      <span class="document-folder-mark">${escapeHTML(courseFolderShort(courseId))}</span>
      <span class="document-folder-copy">
        <strong>${escapeHTML(courseFolderTitle(courseId))}</strong>
        <small>${folderDocs.length} document${folderDocs.length > 1 ? 's' : ''} · ${categoryCount} rubrique${categoryCount > 1 ? 's' : ''}</small>
        ${contextLabel ? `<em>${escapeHTML(contextLabel)}</em>` : ''}
      </span>
      <span class="document-folder-arrow" aria-hidden="true">→</span>
    </button>`;
  }).join('')}</div>`;
}

function renderCategoryFolders(documents, courseId) {
  const courseDocs = documentsForCourse(documents, courseId);
  return `<div class="document-category-grid">${DOCUMENT_CATEGORIES.map(function categoryFolder(category) {
    const count = courseDocs.filter(function byCategory(doc) { return doc.categoryId === category.id; }).length;
    return `<button class="document-category-card ${count ? '' : 'empty'}" type="button" data-doc-action="open-category" data-category-id="${category.id}">
      <span class="document-category-mark">${category.short}</span>
      <span class="document-category-copy"><strong>${category.label}</strong><small>${category.description}</small></span>
      <span class="document-category-count">${count}</span>
    </button>`;
  }).join('')}</div>`;
}

function renderDocumentItems(documents, role) {
  if (!documents.length) return '<div class="document-empty"><strong>Aucun document dans cette rubrique</strong><span>Le contenu apparaîtra ici automatiquement après génération ou ajout par le formateur.</span></div>';
  return `<div class="document-item-list">${documents.map(function renderDoc(doc) {
    const category = documentCategory(doc.categoryId);
    const recipient = role === 'trainer' && doc.targetUserId ? remoteUsers.find(function findUser(user) { return user.id === doc.targetUserId; }) : null;
    const linkedSession = role === 'trainer' && doc.sessionId ? remoteSessions.find(function findSession(item) { return item.id === doc.sessionId; }) : null;
    ...
    return `<article class="document-item-card">
      <span class="document-item-mark">${category.short}</span>
      <div class="document-item-copy">
        <div class="document-item-topline"><span>${escapeHTML(category.label)}</span>${isRecentDocument(doc.createdAt) ? '<b>Nouveau</b>' : ''}</div>
        <strong>${escapeHTML(doc.name)}</strong>
        <p>${escapeHTML(doc.status)}${documentDateLabel(doc.createdAt) ? ' · ' + escapeHTML(documentDateLabel(doc.createdAt)) : ''}</p>
        ${recipient ? `<small>Stagiaire : ${escapeHTML(recipient.name || recipient.email)}</small>` : ''}
        ${linkedSession ? `<small>Session : ${escapeHTML(linkedSession.title || 'Session de formation')}</small>` : ''}
      </div>
      <div class="document-item-actions">
        ${doc.href ? `<a class="primary-button compact" href="${escapeHTML(doc.href)}" target="_blank" rel="noopener">Ouvrir</a>` : '<span class="badge-soft">Lien indisponible</span>'}
        ${deleteButton}
      </div>
    </article>`;
  }).join('')}</div>`;
...
function renderDocumentBreadcrumb(parts) {
  return `<div class="document-breadcrumb"><button type="button" data-doc-action="library-home">Documents</button>${parts.map(function part(item) {
    return `<span>›</span><strong>${escapeHTML(item)}</strong>`;
  }).join('')}</div>`;
}

function renderCourseLibrary(documents, role, preferredCourseIds, contextParts) {
  const selectedCourse = documentLibraryState.courseId;
  const selectedCategory = documentLibraryState.categoryId;
  const courseIds = courseIdsForDocuments(documents, preferredCourseIds);

  if (!selectedCourse) {
    return `${renderDocumentBreadcrumb(contextParts || [])}
      <div class="document-library-heading"><div><p class="eyebrow">Étape 1</p><h2>Choisir une formation</h2><p>Chaque formation possède son propre espace documentaire.</p></div></div>
      ${renderCourseFolders(documents, courseIds, contextParts && contextParts.length ? contextParts[contextParts.length - 1] : '')}`;
  }

  if (!selectedCategory) {
    return `${renderDocumentBreadcrumb((contextParts || []).concat([courseFolderTitle(selectedCourse)]))}
      <button class="document-back-button" type="button" data-doc-action="back-to-courses">← Retour aux formations</button>
      <div class="document-library-heading"><div><p class="eyebrow">Étape 2</p><h2>${escapeHTML(courseFolderTitle(selectedCourse))}</h2><p>Choisis la rubrique de documents que tu souhaites consulter.</p></div><span class="document-total-badge">${documentsForCourse(documents, selectedCourse).length} document${documentsForCourse(documents, selectedCourse).length > 1 ? 's' : ''}</span></div>
      ${renderCategoryFolders(documents, selectedCourse)}`;
  }

  const category = documentCategory(selectedCategory);
  const visibleDocs = documentsForCourse(documents, selectedCourse).filter(function categoryFilter(doc) { return doc.categoryId === selectedCategory; });
  return `${renderDocumentBreadcrumb((contextParts || []).concat([courseFolderTitle(selectedCourse), category.label]))}
    <button class="document-back-button" type="button" data-doc-action="back-to-categories">← Retour aux rubriques</button>
    <div class="document-library-heading"><div><p class="eyebrow">${escapeHTML(courseFolderTitle(selectedCourse))}</p><h2>${escapeHTML(category.label)}</h2><p>${escapeHTML(category.description)}</p></div><span class="document-total-badge">${visibleDocs.length}</span></div>
    ${renderDocumentItems(visibleDocs, role)}`;
}

function renderTrainerStudentLibrary(documents) {
  const students = remoteUsers.filter(function onlyStudents(user) { return user.role === 'student'; });
  const selectedStudent = students.find(function findStudent(user) { return user.id === documentLibraryState.studentId; });
  if (!selectedStudent) {
    return `${renderDocumentBreadcrumb(['Par stagiaire'])}
      <div class="document-library-heading"><div><p class="eyebrow">Classement nominatif</p><h2>Choisir un stagiaire</h2><p>Retrouve ses documents personnels et les ressources partagées de ses formations.</p></div></div>
      <div class="document-folder-grid">${students.length ? students.map(function studentFolder(student) {
        const count = documentsForStudent(documents, student).length;
        return `<button class="document-folder-card" type="button" data-doc-action="open-student" data-student-id="${escapeHTML(student.id)}">
          <span class="document-folder-mark person">${escapeHTML((student.name || 'S').slice(0, 2).toUpperCase())}</span>
          <span class="document-folder-copy"><strong>${escapeHTML(student.name || 'Stagiaire')}</strong><small>${escapeHTML(student.email || '')}</small><em>${count} document${count > 1 ? 's' : ''}</em></span>
          <span class="document-folder-arrow">→</span>
        </button>`;
      }).join('') : '<div class="document-empty"><strong>Aucun stagiaire</strong><span>Crée d’abord un compte stagiaire.</span></div>'}</div>`;
  }
  const studentDocs = documentsForStudent(documents, selectedStudent);
  return `<button class="document-back-button" type="button" data-doc-action="back-to-students">← Retour aux stagiaires</button>${renderCourseLibrary(studentDocs, 'trainer', selectedStudent.courses || [], ['Par stagiaire', selectedStudent.name || selectedStudent.email || 'Stagiaire'])}`;
}

function renderTrainerSessionLibrary(documents) {
  const selectedSession = remoteSessions.find(function findSession(item) { return item.id === documentLibraryState.sessionId; });
  if (!selectedSession) {
    return `${renderDocumentBreadcrumb(['Par session'])}
      <div class="document-library-heading"><div><p class="eyebrow">Dossiers de session</p><h2>Choisir une session</h2><p>Consulte les documents générés ou associés à une session précise.</p></div></div>
      <div class="document-folder-grid">${remoteSessions.length ? remoteSessions.map(function sessionFolder(item) {
        const count = documents.filter(function bySession(doc) { return doc.sessionId === item.id; }).length;
        return `<button class="document-folder-card" type="button" data-doc-action="open-session" data-session-id="${escapeHTML(item.id)}">
          <span class="document-folder-mark session">${escapeHTML(courseFolderShort(item.course_id))}</span>
          <span class="document-folder-copy"><strong>${escapeHTML(item.title || 'Session de formation')}</strong><small>${escapeHTML(courseTitle(item.course_id))} · ${escapeHTML(formatFrenchDate(item.start_date))}</small><em>${count} document${count > 1 ? 's' : ''}</em></span>
          <span class="document-folder-arrow">→</span>
        </button>`;
      }).join('') : '<div class="document-empty"><strong>Aucune session</strong><span>Les dossiers de session apparaîtront après leur création.</span></div>'}</div>`;
  }
  const sessionDocs = documents.filter(function bySession(doc) { return doc.sessionId === selectedSession.id; });
  if (!documentLibraryState.categoryId) {
    return `${renderDocumentBreadcrumb(['Par session', selectedSession.title || 'Session'])}
      <button class="document-back-button" type="button" data-doc-action="back-to-sessions">← Retour aux sessions</button>
      <div class="document-library-heading"><div><p class="eyebrow">${escapeHTML(courseTitle(selectedSession.course_id))}</p><h2>${escapeHTML(selectedSession.title || 'Session de formation')}</h2><p>${escapeHTML(formatFrenchDate(selectedSession.start_date))} → ${escapeHTML(formatFrenchDate(selectedSession.end_date))}${selectedSession.location ? ' · ' + escapeHTML(selectedSession.location) : ''}</p></div><span class="document-total-badge">${sessionDocs.length} document${sessionDocs.length > 1 ? 's' : ''}</span></div>
      <div class="document-category-grid">${DOCUMENT_CATEGORIES.map(function sessionCategory(category) {
        const count = sessionDocs.filter(function byCategory(doc) { return doc.categoryId === category.id; }).length;
        return `<button class="document-category-card ${count ? '' : 'empty'}" type="button" data-doc-action="open-category" data-category-id="${category.id}"><span class="document-category-mark">${category.short}</span><span class="document-category-copy"><strong>${category.label}</strong><small>${category.description}</small></span><span class="document-category-count">${count}</span></button>`;
      }).join('')}</div>`;
  }
  const category = documentCategory(documentLibraryState.categoryId);
  const categoryDocs = sessionDocs.filter(function byCategory(doc) { return doc.categoryId === category.id; });
  return `${renderDocumentBreadcrumb(['Par session', selectedSession.title || 'Session', category.label])}
    <button class="document-back-button" type="button" data-doc-action="back-session-categories">← Retour aux rubriques</button>
    <div class="document-library-heading"><div><p class="eyebrow">${escapeHTML(selectedSession.title || 'Session')}</p><h2>${escapeHTML(category.label)}</h2><p>${escapeHTML(category.description)}</p></div><span class="document-total-badge">${categoryDocs.length}</span></div>
    ${renderDocumentItems(categoryDocs, 'trainer')}`;
}

function documentsView(role) {
  const allDocs = remoteDocuments.map(normalizeLibraryDocument);
  const trainerUpload = role === 'trainer' ? trainerDocumentUpload() : '';
  const emptyRemoteNotice = isRemoteSession() ? '' : '<div class="info-banner"><strong>Mode local actif</strong><span>Renseigne Supabase pour afficher les documents privés et partagés.</span></div>';
  let libraryContent = '';

  if (role === 'trainer') {
    if (documentLibraryState.view === 'student') libraryContent = renderTrainerStudentLibrary(allDocs);
    else if (documentLibraryState.view === 'session') libraryContent = renderTrainerSessionLibrary(allDocs);
    else libraryContent = renderCourseLibrary(allDocs, role, courses.map(function courseId(item) { return item.id; }), ['Par formation']);
  } else {
    const allowedCourseIds = session && Array.isArray(session.courses) ? session.courses : [];
    libraryContent = renderCourseLibrary(allDocs, role, allowedCourseIds, []);
  }

  return appShell(template`
    <section class="glass-panel content-card document-library-shell fade-in">
      <div class="section-heading"><p class="eyebrow">Bibliothèque documentaire</p><h1>${role === 'trainer' ? 'Documents de formation' : 'Mes documents'}</h1></div>
      <p class="lead small">${role === 'trainer' ? 'Classement automatique par formation, catégorie, stagiaire ou session.' : 'Choisis une formation, puis une rubrique pour retrouver rapidement chaque document.'}</p>
      ${emptyRemoteNotice}
      ${role === 'trainer' ? renderDocumentViewTabs(documentLibraryState.view) : ''}
      ${trainerUpload}
      <div class="document-library-content">${libraryContent}</div>
    </section>
  `, role);
}
...
function trainerDocumentUpload() {
  ...
  const sessionOptions = remoteSessions.map(function renderSessionOption(item) {
    return `<option value="${escapeHTML(item.id)}">${escapeHTML(item.title || 'Session')} — ${escapeHTML(courseTitle(item.course_id))}</option>`;
  }).join('');
  const remoteNotice = isRemoteSession()
    ? '<p class="lead small">Le document sera classé automatiquement grâce à sa formation, son type, son stagiaire et sa session.</p>'
    : '<p class="lead small">Connexion Supabase requise pour activer l’upload sécurisé des documents.</p>';
  return template`
    <details class="document-upload-disclosure glass-panel">
      <summary><span>＋</span><strong>Ajouter un document sécurisé</strong><small>Ouvrir le formulaire d’envoi</small></summary>
      <div class="upload-zone">
        ${remoteNotice}
        <form id="document-upload-form" class="auth-form compact-form">
          <label class="full-row">Titre du document<input name="title" required placeholder="Support pédagogique - Outils bureautiques" /></label>
          <label>Type de document
            <select name="type" required>
              <option value="convocation">Convocation</option>
              <option value="livret">Livret pédagogique</option>
              <option value="support">Support pédagogique</option>
              <option value="workbook">Cahier d’exercices</option>
              <option value="glossary">Glossaire</option>
              <option value="memo">Fiche mémo</option>
          ...
          <label>Stagiaire destinataire
            <select name="targetUserId">
              <option value="">Partagé avec les stagiaires de la formation</option>
              ${studentOptions}
            </select>
          </label>
          <label>Session associée (facultatif)
            <select name="sessionId">
              <option value="">Aucune session précise</option>
              ${sessionOptions}
            </select>
          </label>
          <label class="full-row">Fichier à envoyer<input name="file" type="file" accept="application/pdf,image/png,image/jpeg" required /></label>
          <p class="form-help full-row">Pour une convocation ou une attestation nominative, choisis obligatoirement un stagiaire. La session est facultative pour les documents pédagogiques partagés.</p>
          <p class="form-error full-row" role="alert" hidden></p>
          <button class="primary-button full-row" type="submit" ${isRemoteSession() ? '' : 'disabled'}>Envoyer et classer automatiquement</button>
        </form>
      </div>
    </details>
  `;
...
function trainerOverview() {
  ...
  return appShell(template`
    ...
    <section class="action-grid fade-in delay-4" aria-label="Actions formateur">
      ...
      <a class="action-card glass-panel" href="#/trainer/attendance">${icons.check}<strong>Faire signer</strong><span>Émargement matin et après-midi</span></a>
...
function attendanceSessionDates(item) {
  const dates = [];
  if (!item || !item.start_date) return dates;
  const start = new Date(`${item.start_date}T12:00:00`);
  const end = new Date(`${item.end_date || item.start_date}T12:00:00`);
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) return [item.start_date];
  ...
  return dates.length ? dates : [item.start_date];
}

function attendanceRecordFor(sessionId, attendanceDate, period, participantId) {
  return remoteAttendanceSignatures.find(function matching(record) {
    return record.sessionId === sessionId && record.attendanceDate === attendanceDate && record.period === period && record.participantId === participantId;
  }) || null;
}
...
function attendanceStatusLabel(status) {
  ...
  if (status === 'refused') return 'Refus';
  return 'À signer';
}

function attendanceStatusClass(status) {
  if (status === 'signed') return 'is-signed';
  if (status === 'absent') return 'is-absent';
  if (status === 'refused') return 'is-refused';
  return 'is-pending';
}

function attendanceSignedTime(value) {
  if (!value) return '';
  try { return new Intl.DateTimeFormat('fr-FR', { hour: '2-digit', minute: '2-digit' }).format(new Date(value)); } catch (_) { return ''; }
}

function trainerAttendanceView() {
  if (!remoteSessions.length) {
    return appShell(template`
      <section class="hero-card glass-panel fade-in"><p class="eyebrow">Émargement numérique</p><h1>Aucune session disponible</h1><p>Crée d’abord une session et rattache ses stagiaires.</p><a class="primary-button" href="#/trainer/sessions">Créer une session</a></section>
    `, 'trainer');
  }

  let selected = remoteSessions.find(function selectedSession(item) { return item.id === attendanceUiState.sessionId; }) || remoteSessions[0];
  attendanceUiState.sessionId = selected.id;
  const dates = attendanceSessionDates(selected);
  if (dates.indexOf(attendanceUiState.attendanceDate) === -1) attendanceUiState.attendanceDate = dates[0] || selected.start_date;
  const period = attendanceUiState.period === 'afternoon' ? 'afternoon' : 'morning';
  const students = selected.students || [];
  const records = students.map(function attachRecord(student) {
    return { student, record: attendanceRecordFor(selected.id, attendanceUiState.attendanceDate, period, student.student_id) };
  });
  const completedCount = records.filter(function completed(item) { return Boolean(item.record); }).length;
  const pendingCount = Math.max(0, students.length - completedCount);
  const trainerRecord = attendanceRecordFor(selected.id, attendanceUiState.attendanceDate, period, session.id);
  const allStudentsDone = students.length > 0 && pendingCount === 0;
  const pdfReady = allStudentsDone && trainerRecord && trainerRecord.status === 'signed';
  const signingStudent = students.find(function findStudent(row) { return row.student_id === attendanceUiState.signingParticipantId; });
  const signingRole = attendanceUiState.signingParticipantRole;
  const modalName = signingRole === 'trainer'
    ? (session.name || session.email || 'Formateur')
    : (signingStudent && signingStudent.profile ? signingStudent.profile.name : 'Stagiaire');

  return appShell(template`
    <section class="hero-card glass-panel fade-in attendance-hero">
      <div><p class="eyebrow">V6.3 · Émargement itinérant</p><h1>Faire signer les stagiaires</h1><p>Le stagiaire signe directement sur le téléphone ou la tablette du formateur. Aucun accès stagiaire n’est nécessaire.</p></div>
      <div class="attendance-hero-progress"><span>${completedCount}/${students.length}</span><strong>stagiaires traités</strong><small>${pendingCount ? `${pendingCount} restant${pendingCount > 1 ? 's' : ''}` : 'Créneau complet'}</small></div>
    </section>

    ${attendanceUiState.notice ? `<div class="attendance-notice glass-panel">${escapeHTML(attendanceUiState.notice)}</div>` : ''}

    <section class="glass-panel content-card fade-in delay-1">
      <div class="section-heading"><p class="eyebrow">1 · Session</p><h2>Choisir la formation</h2></div>
      <div class="attendance-session-grid">
        ${remoteSessions.map(function renderSessionChoice(item) {
          return `<button class="attendance-session-choice ${item.id === selected.id ? 'active' : ''}" type="button" data-action="attendance-select-session" data-session-id="${escapeHTML(item.id)}"><strong>${escapeHTML(item.title || courseTitle(item.course_id))}</strong><span>${escapeHTML(courseTitle(item.course_id))}</span><small>${formatFrenchDate(item.start_date)} → ${formatFrenchDate(item.end_date)} · ${item.studentCount || 0} stagiaire${(item.studentCount || 0) > 1 ? 's' : ''}</small></button>`;
        }).join('')}
      </div>
    </section>

    <section class="glass-panel content-card fade-in delay-2">
      <div class="section-heading"><p class="eyebrow">2 · Créneau</p><h2>Date et demi-journée</h2></div>
      <div class="attendance-slot-controls">
        <div class="attendance-date-tabs">${dates.map(function renderDate(date) { return `<button class="attendance-date-tab ${date === attendanceUiState.attendanceDate ? 'active' : ''}" type="button" data-action="attendance-select-date" data-date="${escapeHTML(date)}">${formatFrenchDate(date)}</button>`; }).join('')}</div>
        <div class="attendance-period-tabs">
          <button class="attendance-period-tab ${period === 'morning' ? 'active' : ''}" type="button" data-action="attendance-select-period" data-period="morning">Matin</button>
          <button class="attendance-period-tab ${period === 'afternoon' ? 'active' : ''}" type="button" data-action="attendance-select-period" data-period="afternoon">Après-midi</button>
        </div>
      </div>
    </section>

    <section class="glass-panel content-card fade-in delay-3">
      <div class="attendance-list-heading"><div class="section-heading"><p class="eyebrow">3 · Signatures</p><h2>${escapeHTML(selected.title || 'Session')}</h2></div><span class="attendance-slot-badge">${period === 'morning' ? 'Matin' : 'Après-midi'} · ${formatFrenchDate(attendanceUiState.attendanceDate)}</span></div>
      <div class="attendance-roster">
        ${records.map(function renderAttendanceRow(item, index) {
          const student = item.student;
          const profile = student.profile || {};
          const status = item.record ? item.record.status : '';
          return `<article class="attendance-person-card ${attendanceStatusClass(status)}">
            <div class="attendance-index">${index + 1}</div>
            <div class="attendance-person-copy"><strong>${escapeHTML(profile.name || 'Stagiaire')}</strong><span>${escapeHTML(profile.email || '')}</span><small>${item.record ? `${attendanceStatusLabel(status)}${item.record.signedAt ? ' à ' + attendanceSignedTime(item.record.signedAt) : ''}` : 'En attente de passage'}</small></div>
            <span class="attendance-status-pill">${attendanceStatusLabel(status)}</span>
            <button class="${item.record ? 'ghost-button' : 'primary-button'} compact" type="button" data-action="attendance-open-signature" data-participant-id="${escapeHTML(student.student_id)}" data-participant-role="student">${item.record ? 'Modifier' : 'Faire signer'}</button>
          </article>`;
        }).join('') || '<div class="document-empty"><strong>Aucun stagiaire</strong><span>Rattache au moins un stagiaire à cette session.</span></div>'}
      </div>
    </section>

    <section class="attendance-final-grid fade-in delay-4">
      <article class="glass-panel content-card">
        <div class="section-heading"><p class="eyebrow">4 · Validation</p><h2>Signature formateur</h2></div>
        <p>${trainerRecord ? `Signée par ${escapeHTML(trainerRecord.participantName || session.name || 'Formateur')} à ${attendanceSignedTime(trainerRecord.signedAt)}.` : allStudentsDone ? 'Tous les stagiaires ont un statut. Tu peux maintenant signer le créneau.' : 'Termine d’abord le passage auprès de chaque stagiaire.'}</p>
        <button class="primary-button" type="button" data-action="attendance-open-signature" data-participant-id="${escapeHTML(session.id)}" data-participant-role="trainer" ${allStudentsDone ? '' : 'disabled'}>${trainerRecord ? 'Modifier ma signature' : 'Signer en tant que formateur'}</button>
      </article>
      <article class="glass-panel content-card accent-card">
        <div class="section-heading"><p class="eyebrow">5 · Archivage</p><h2>PDF signé</h2></div>
        <p>Le PDF contient les statuts, les signatures, les heures et la validation finale du formateur. Il sera classé automatiquement dans Documents → Émargements.</p>
        <button class="primary-button" type="button" data-action="attendance-generate-pdf" ${pdfReady ? '' : 'disabled'}>Générer et archiver le PDF</button>
        <small class="form-help">${pdfReady ? 'Le créneau est complet et prêt à être archivé.' : 'Le bouton sera disponible quand tous les stagiaires et le formateur auront signé ou été renseignés.'}</small>
      </article>
    </section>

    ${attendanceUiState.signingParticipantId ? `<div class="signature-modal" role="dialog" aria-modal="true" aria-labelledby="signature-modal-title">
      <button class="signature-modal-backdrop" type="button" data-action="attendance-close-signature" aria-label="Fermer"></button>
      <section class="signature-modal-card glass-panel">
        <div class="signature-modal-head"><div><p class="eyebrow">${signingRole === 'trainer' ? 'Validation du créneau' : 'Signature du stagiaire'}</p><h2 id="signature-modal-title">${escapeHTML(modalName)}</h2><p>${period === 'morning' ? 'Matin' : 'Après-midi'} · ${formatFrenchDate(attendanceUiState.attendanceDate)}</p></div><button class="icon-button" type="button" data-action="attendance-close-signature" aria-label="Fermer">${icons.close}</button></div>
        <div class="signature-pad-wrap"><canvas id="attendance-signature-canvas" class="signature-pad" aria-label="Zone de signature"></canvas><span>Signez dans cette zone</span></div>
        <div class="signature-actions">
          <button class="ghost-button" type="button" data-action="attendance-clear-signature">Effacer</button>
          ${signingRole === 'student' ? '<button class="ghost-button danger-soft" type="button" data-action="attendance-save-status" data-status="absent">Absent</button><button class="ghost-button warning-soft" type="button" data-action="attendance-save-status" data-status="refused">Refus de signer</button>' : ''}
          <button class="primary-button" type="button" data-action="attendance-save-signature">Valider la signature</button>
        </div>
      </section>
    </div>` : ''}
  `, 'trainer');
        ...
        return `<article class="session-card">
          <div class="session-card-head">
            ...
            <div class="session-actions">
              <button class="primary-button compact" type="button" data-action="open-attendance" data-session-id="${escapeHTML(item.id)}">Faire signer les stagiaires</button>
              <button class="ghost-button compact" type="button" data-action="generate-attendance" data-mode="journee" data-session-id="${escapeHTML(item.id)}">Feuille vierge</button>
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-action="logout"]').forEach(function bindLogout(button) {
    button.addEventListener('click', async function doLogout() {
      ...
      remoteUsers = [];
      remoteSessions = [];
      documentLibraryState = { view: 'course', courseId: '', categoryId: '', studentId: '', sessionId: '' };
      drawerOpen = false;
      navigate('/login');
    });
  });
  ...
  document.querySelectorAll('[data-action="open-attendance"]').forEach(function bindOpenAttendance(button) {
    button.addEventListener('click', function openAttendance() {
      attendanceUiState.sessionId = button.dataset.sessionId || '';
      attendanceUiState.attendanceDate = '';
      attendanceUiState.period = 'morning';
      attendanceUiState.notice = '';
      navigate('/trainer/attendance');
    });
  });

  document.querySelectorAll('[data-action="attendance-select-session"]').forEach(function bindAttendanceSession(button) {
    button.addEventListener('click', function selectAttendanceSession() {
      attendanceUiState.sessionId = button.dataset.sessionId || '';
      attendanceUiState.attendanceDate = '';
      attendanceUiState.signingParticipantId = '';
      attendanceUiState.notice = '';
      render();
    });
  });

  document.querySelectorAll('[data-action="attendance-select-date"]').forEach(function bindAttendanceDate(button) {
    button.addEventListener('click', function selectAttendanceDate() {
      attendanceUiState.attendanceDate = button.dataset.date || '';
      attendanceUiState.signingParticipantId = '';
      attendanceUiState.notice = '';
      render();
    });
  ...
  document.querySelectorAll('[data-action="attendance-select-period"]').forEach(function bindAttendancePeriod(button) {
    button.addEventListener('click', function selectAttendancePeriod() {
      attendanceUiState.period = button.dataset.period === 'afternoon' ? 'afternoon' : 'morning';
      attendanceUiState.signingParticipantId = '';
      attendanceUiState.notice = '';
      render();
    });
  ...
  document.querySelectorAll('[data-action="attendance-open-signature"]').forEach(function bindAttendanceOpenSignature(button) {
    button.addEventListener('click', function openSignature() {
      attendanceUiState.signingParticipantId = button.dataset.participantId || '';
      attendanceUiState.signingParticipantRole = button.dataset.participantRole || 'student';
      attendanceUiState.notice = '';
      render();
    });
  });

  document.querySelectorAll('[data-action="attendance-close-signature"]').forEach(function bindAttendanceCloseSignature(button) {
    button.addEventListener('click', function closeSignature() {
      attendanceUiState.signingParticipantId = '';
      attendanceUiState.signingParticipantRole = '';
      render();
    });
  });

  const signatureCanvas = document.querySelector('#attendance-signature-canvas');
  if (signatureCanvas) {
    const rect = signatureCanvas.getBoundingClientRect();
    const ratio = Math.min(2, window.devicePixelRatio || 1);
    signatureCanvas.width = Math.max(600, Math.round(rect.width * ratio));
    signatureCanvas.height = Math.max(280, Math.round(rect.height * ratio));
    ...
    signatureCanvas.addEventListener('pointerdown', function startSignature(event) {
      drawing = true;
    ...
    signatureCanvas.addEventListener('pointermove', function drawSignature(event) {
      if (!drawing) return;
      const pos = point(event);
      context.lineTo(pos.x, pos.y);
      context.stroke();
      signatureCanvas.dataset.hasInk = 'true';
      event.preventDefault();
    });
    ...
    const clearButton = document.querySelector('[data-action="attendance-clear-signature"]');
    if (clearButton) clearButton.addEventListener('click', function clearSignature() {
      context.clearRect(0, 0, signatureCanvas.width, signatureCanvas.height);
      context.fillStyle = '#ffffff';
      context.fillRect(0, 0, signatureCanvas.width, signatureCanvas.height);
      signatureCanvas.dataset.hasInk = '';
    });
  }

  async function saveAttendanceEntry(status, signatureData) {
    if (!isRemoteSession() || !attendanceUiState.signingParticipantId) return;
    const selectedSession = remoteSessions.find(function findSession(item) { return item.id === attendanceUiState.sessionId; });
    if (!selectedSession) return;
    const isTrainer = attendanceUiState.signingParticipantRole === 'trainer';
    const student = (selectedSession.students || []).find(function findStudent(row) { return row.student_id === attendanceUiState.signingParticipantId; });
    const participantName = isTrainer ? (session.name || session.email || 'Formateur') : (student && student.profile ? student.profile.name : 'Stagiaire');
    try {
      await upsertRemoteAttendanceSignature(session, {
        sessionId: selectedSession.id,
        attendanceDate: attendanceUiState.attendanceDate,
        period: attendanceUiState.period,
        participantId: attendanceUiState.signingParticipantId,
        participantRole: isTrainer ? 'trainer' : 'student',
        participantName,
        status,
        signatureData: signatureData || ''
      });
      remoteAttendanceSignatures = await listRemoteAttendanceSignatures(session);
      remoteSessions = await listRemoteSessions(session);
      trackEvent('attendance_signature_saved', { sessionId: selectedSession.id, participantId: attendanceUiState.signingParticipantId, role: isTrainer ? 'trainer' : 'student', status, date: attendanceUiState.attendanceDate, period: attendanceUiState.period });
      if (!isTrainer) {
        const students = selectedSession.students || [];
        const currentIndex = students.findIndex(function current(row) { return row.student_id === attendanceUiState.signingParticipantId; });
        let next = null;
        for (let offset = 1; offset <= students.length; offset += 1) {
          const candidate = students[(currentIndex + offset) % students.length];
          if (!attendanceRecordFor(selectedSession.id, attendanceUiState.attendanceDate, attendanceUiState.period, candidate.student_id)) {
            next = candidate;
            break;
          }
        }
        if (next) {
          attendanceUiState.signingParticipantId = next.student_id;
          attendanceUiState.signingParticipantRole = 'student';
          attendanceUiState.notice = `${attendanceStatusLabel(status)} enregistré pour ${participantName}. Passage automatique au stagiaire suivant.`;
        } else {
          attendanceUiState.signingParticipantId = '';
          attendanceUiState.signingParticipantRole = '';
          attendanceUiState.notice = 'Tous les stagiaires ont été traités pour ce créneau. La signature formateur est maintenant disponible.';
        }
      } else {
        attendanceUiState.signingParticipantId = '';
        attendanceUiState.signingParticipantRole = '';
        attendanceUiState.notice = 'Signature formateur enregistrée. Le PDF signé peut maintenant être généré.';
      }
      render();
    } catch (error) {
      window.alert(error.message || 'Enregistrement de l’émargement impossible. Exécute le fichier SQL V6.3 dans Supabase si nécessaire.');
    }
  }

  const saveSignatureButton = document.querySelector('[data-action="attendance-save-signature"]');
  if (saveSignatureButton) saveSignatureButton.addEventListener('click', async function saveSignature() {
    const canvas = document.querySelector('#attendance-signature-canvas');
    if (!canvas || canvas.dataset.hasInk !== 'true') {
      window.alert('La zone de signature est vide.');
      return;
    }
    saveSignatureButton.disabled = true;
    await saveAttendanceEntry('signed', canvas.toDataURL('image/png', 0.9));
  });

  document.querySelectorAll('[data-action="attendance-save-status"]').forEach(function bindAttendanceStatus(button) {
    button.addEventListener('click', async function saveStatus() {
      const label = button.dataset.status === 'absent' ? 'absent' : 'en refus de signer';
      if (!window.confirm(`Confirmer que ce stagiaire est ${label} ?`)) return;
      button.disabled = true;
      await saveAttendanceEntry(button.dataset.status, '');
    });
  });

  const generateSignedPdfButton = document.querySelector('[data-action="attendance-generate-pdf"]');
  if (generateSignedPdfButton) generateSignedPdfButton.addEventListener('click', async function generateSignedPdf() {
    if (!isRemoteSession()) return;
    generateSignedPdfButton.disabled = true;
    const originalLabel = generateSignedPdfButton.textContent;
    generateSignedPdfButton.textContent = 'Génération du PDF…';
    try {
      const documentRow = await generateRemoteSignedAttendanceSheet(session, attendanceUiState.sessionId, attendanceUiState.attendanceDate, attendanceUiState.period);
      remoteDocuments = await listRemoteDocuments(session);
      attendanceUiState.notice = 'Feuille d’émargement signée générée et classée dans Documents → Émargements.';
      trackEvent('signed_attendance_pdf_generated', { sessionId: attendanceUiState.sessionId, date: attendanceUiState.attendanceDate, period: attendanceUiState.period, documentId: documentRow && documentRow.id });
      render();
    } catch (error) {
      window.alert(error.message || 'Génération du PDF signé impossible.');
      generateSignedPdfButton.disabled = false;
      generateSignedPdfButton.textContent = originalLabel;
    }
  });
  ...
  document.querySelectorAll('[data-doc-action]').forEach(function bindDocumentLibrary(button) {
    button.addEventListener('click', function navigateDocumentLibrary() {
      const action = button.dataset.docAction;
      if (action === 'set-view') {
        documentLibraryState = { view: button.dataset.view || 'course', courseId: '', categoryId: '', studentId: '', sessionId: '' };
      } else if (action === 'open-course') {
        documentLibraryState.courseId = button.dataset.courseId || '';
        documentLibraryState.categoryId = '';
      } else if (action === 'open-category') {
        documentLibraryState.categoryId = button.dataset.categoryId || '';
      } else if (action === 'open-student') {
        documentLibraryState.studentId = button.dataset.studentId || '';
        documentLibraryState.courseId = '';
        documentLibraryState.categoryId = '';
      } else if (action === 'open-session') {
        documentLibraryState.sessionId = button.dataset.sessionId || '';
        documentLibraryState.categoryId = '';
      } else if (action === 'back-to-courses') {
        documentLibraryState.courseId = '';
        documentLibraryState.categoryId = '';
      } else if (action === 'back-to-categories' || action === 'back-session-categories') {
        documentLibraryState.categoryId = '';
      } else if (action === 'back-to-students') {
        documentLibraryState.studentId = '';
        documentLibraryState.courseId = '';
        documentLibraryState.categoryId = '';
      } else if (action === 'back-to-sessions') {
        documentLibraryState.sessionId = '';
        documentLibraryState.categoryId = '';
      } else if (action === 'library-home') {
        documentLibraryState.courseId = '';
        documentLibraryState.categoryId = '';
        if (documentLibraryState.view === 'student') documentLibraryState.studentId = '';
        if (documentLibraryState.view === 'session') documentLibraryState.sessionId = '';
      }
      render();
    });
  });
...
function guarded(route) {
  ...
  switch (route) {
    ...
    case '/trainer/attendance': return trainerAttendanceView();
