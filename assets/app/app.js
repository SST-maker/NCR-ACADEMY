...
function ticketList(role) {
  ...
  return `<div class="ticket-list">${tickets.map(function renderTicket(ticket) {
    ...
    const trainerActions = role === 'trainer'
...
function trainerSettings() {
  ...
  return appShell(template`
    <section class="section-grid two fade-in">
      <article class="glass-panel content-card">
        <div class="section-heading"><p class="eyebrow">Stagiaires</p><h1>Créer un accès stagiaire</h1></div>
        ...
        <form id="student-create-form" class="auth-form compact-form">
          ...
          <label>Formations accessibles
            <select name="courseIds" multiple size="5">${courseOptions}</select>
          </label>
      ...
      <article class="glass-panel content-card">
        <div class="section-heading"><p class="eyebrow">Liste</p><h2>Stagiaires existants</h2></div>
        ${studentsList}
      </article>
    </section>

    <section class="glass-panel content-card fade-in delay-1">
      <div class="section-heading"><p class="eyebrow">Important</p><h2>Fonctionnement des tickets</h2></div>
      <ul class="check-list clean">
        <li><strong>Côté stagiaire</strong><span>Le ticket est envoyé depuis Contact et assistance.</span></li>
        <li><strong>Côté formateur</strong><span>Le ticket apparaît dans Assistance avec le nom, l’email, le message et le statut.</span></li>
        <li><strong>Statut</strong><span>Tu peux passer un ticket en ouvert, en cours, traité ou fermé.</span></li>
      </ul>
    </section>
  `, 'trainer');
}
