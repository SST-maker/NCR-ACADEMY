...
function trainerOverview() {
  ...
  return appShell(template`
    <section class="hero-card glass-panel fade-in">
      ...
      <p>Vue rapide sur les tickets, les sessions, les stagiaires et les documents à suivre.</p>
    </section>
    ...
    <section class="quick-grid fade-in delay-1">
      ...
      <article class="glass-panel metric-card"><span class="metric">${certificatesGenerated}</span><strong>Attestations</strong><small>Générées</small></article>
    </section>
    ...
    <section class="section-grid two fade-in delay-2">
      <article class="glass-panel content-card">
        ...
        <ul class="check-list clean">
          ...
          <li><strong>Sessions à venir</strong><span>${upcomingSessions.length ? `${upcomingSessions.length} session${upcomingSessions.length > 1 ? 's' : ''} planifiée${upcomingSessions.length > 1 ? 's' : ''}` : 'Aucune session à venir'}</span></li>
        </ul>
      </article>

      <article class="glass-panel content-card">
        <div class="section-heading"><p class="eyebrow">Dernières actions</p><h2>Journal</h2></div>
    ...
    <section class="action-grid fade-in delay-3" aria-label="Actions formateur">
      <a class="action-card glass-panel" href="#/trainer/students">${icons.contact}<strong>Mes stagiaires</strong><span>Créer, modifier et débloquer des formations</span></a>
      ...
      <a class="action-card glass-panel" href="#/trainer/sessions">${icons.documents}<strong>Gérer les sessions</strong><span>Présences, convocations, attestations</span></a>
    </section>
  `, 'trainer');
}
  ...
  return appShell(template`
    <section class="section-grid two fade-in">
      <article class="glass-panel content-card">
        ...
        <p class="lead small">Crée directement un compte stagiaire, son profil et ses inscriptions formation. Le stagiaire pourra ensuite se connecter avec l’email et le mot de passe définis.</p>
